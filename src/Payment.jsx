  ...
  render() { 
    const {deptAmount, trackState,boxClick, interestPaid, invalidPayment, minPayment} = this.props;
     return (
      ...
      <small>Your Payment should be more than 1% <strong>{minPayment}</strong> + 
      the interest <strong>{interestPaid}</strong> (${(Number(minPayment) + Number(interestPaid)).toFixed(2)})
      </small>
      <br/>
      <br/>
